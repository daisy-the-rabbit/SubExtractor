...
public class VideoManagerTitleSet
{
    ...
    public override string ToString()
    {
        ...
        sb.Append($"{this.ifoReader.FileName}\n");
        sb.Append($"Last Sector Title Set 0x{this.lastSectorOfTitleSet:x}\n");
        sb.Append($"Last Sector IFO 0x{this.lastSectorOfIfo:x}\n");
        sb.Append($"Volume Count {this.volumeCount}\n");
        sb.Append($"Volume Number {this.volumeNumber}\n");
        sb.Append($"Side Id {this.sideId}\n");
        sb.Append($"TitleSet Count {this.titleSetCount}\n");
        sb.Append($"Start Sector Menu VOB 0x{this.startSectorMenuVob:x}\n");
        sb.Append($"Language Count {this.languageCount}\n");
        sb.Append($"First Language Code {this.firstLanguageCode}\n");
        sb.Append($"Menu Existance Flag 0x{this.menuExistanceFlag:x}\n");
        sb.Append($"FirstPlay PGC Address {this.firstPlayPGCAddress}\n");

        sb.Append($"{this.titleInfos.Count} Titles\n\n");
        foreach (TitleInfo info in this.titleInfos)
        {
            sb.Append($"Vts {info.VtsNumber} Seq {info.IsSequential} Angles {info.NumberOfAngles} Chapters {info.NumberOfChapters} Title within Vts {info.TitleWithinVts}\n");
            sb.Append($"StartSector {info.StartSector} Command Location {info.CommandLocation}\n\n");
        }

        sb.Append("First Chain\n\n");
        sb.Append(this.firstChain.ToString());
        sb.Append("\n");

        sb.Append($"{this.chains.Length} Chains\n\n");
        for (int index = 0; index < this.chains.Length; index++)
        {
            sb.Append($"Chain {index + 1}\n\n");
        ...
        if (this.audioTrackCount != 0)
        {
            ...
            for (int index = 0; index < this.audioTrackCount; index++)
            {
                ...
                {
                    sb.Append($" {index + 1} ({audio.CodingMode} {DvdLanguageCodes.GetLanguageText(audio.Language)} {audio.Channels}ch)");
                }
                else
                {
                    sb.Append($" {index + 1} ({audio.CodingMode} {DvdLanguageCodes.GetLanguageText(audio.Language)} {audio.Channels}ch {audio.CodeExtension})");
                }
            }
            sb.Append("\n");
        }
        ...
        if (this.subTrackCount != 0)
        {
            ...
            for (int index = 0; index < this.subTrackCount; index++)
            {
                ...
                {
                    sb.Append($" {index + 1} ({DvdLanguageCodes.GetLanguageText(subs.Language)})");
                }
                else
                {
                    sb.Append($" {index + 1} ({DvdLanguageCodes.GetLanguageText(subs.Language)} {subs.CodeExtension})");
                }
            }
            sb.Append("\n");
        }
