        ...
        foreach (PartOfTitle part in parts)
        {
            ...
            if ((this.audioByTrack.Count == 0) && (chain.AudioStreams.Count != 0))
            {
                foreach (KeyValuePair<int, int> audioPair in chain.AudioStreams)
                {
                    ...
                    int streamBase = audio.CodingMode switch
                    {
                        AudioCodingMode.MPEG1 or AudioCodingMode.MPEG2 => 0xC0,
                        AudioCodingMode.LPCM => 0xa0,
                        AudioCodingMode.DTS => 0x88,
                        _ => 0x80,
                    };
                    audioByStream[streamBase + audioPair.Value] = audio;
                }
            }
