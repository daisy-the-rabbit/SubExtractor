...
public class DvdTitleSet : IDvdTitleSet
{
	...
	public override string ToString()
	{
        ...
        sb.Append($"VTS Start Sector 0x{this.vtsStartSector:x}\n");
        ...
        sb.Append($"Start Sector Title VOB 0x{this.startSectorTitleVob:x}\n");
		...
		sb.Append($"{this.menuChains.Length} Menu Chains\n\n");
		for(int index = 0; index < this.menuChains.Length; index++)
		{
			sb.Append($"Menu Chain {index + 1}\n\n");
		...
		if(this.menuSubTrackCount != 0)
		{
			...
			{
				sb.Append($" {1} ({DvdLanguageCodes.GetLanguageText(subs.Language)})");
			}
			else
			{
				sb.Append($" {1} ({DvdLanguageCodes.GetLanguageText(subs.Language)} {subs.CodeExtension})");
			}
			sb.Append("\n\n");
		}

		sb.Append($"{this.titleCount} Titles\n\n");
		for(int titleIndex = 0; titleIndex < this.titleCount; titleIndex++)
		{
			List<PartOfTitle> ptt = this.titleParts[titleIndex];
			sb.Append($"Title {titleIndex + 1}\n\n");
			foreach(PartOfTitle pot in ptt)
			{
				sb.Append($"Chain {pot.ProgramChain} Program {pot.Program}\n");
			}
			sb.Append("\n");
		}
