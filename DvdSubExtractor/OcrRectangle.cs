...
public class OcrRectangle
{
    ...
    List<BlockEncode> reorderedBlocks = [];
    int movieId;
    OcrMap ocrMap;
    ...
    static readonly char[] FirstPassSkippedChars = [':', ';'];
    ...
    {
        int bitmapLength = subBitmap.Stride * subBitmap.Height;
        byte[] bitmapData = new byte[bitmapLength];
        Marshal.Copy(subBitmap.Data, bitmapData, 0, bitmapLength);
        int rectangleOffset = this.sectionRectangle.X + (subBitmap.Stride * this.sectionRectangle.Y);

        ContiguousEncode encode = new ContiguousEncode(bitmapData, rectangleOffset,
