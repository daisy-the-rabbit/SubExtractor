...
public partial class CreateSubtitleFileStep : UserControl, IWizardItem
{
    ...
    List<Control> disabledSrtOptions = [];

    public CreateSubtitleFileStep()
    {
        InitializeComponent();

        this.disabledSrtOptions.AddRange([this.leftCropLabel, this.leftCropUpDown, this.topCropLabel,
            ...
            foreach (CellStartInfo startInfo in this.data.WorkingData.CellStarts)
            {
                ...
                if (isFirstCell)
                {
                    ...
                    this.sectionListTextBox.AppendText(
                        $"{++subDataIndex}: Cell {startInfo.CellType} Pos {startInfo.FilePosition} Audio {audio:f0} Video {video:f0} Offset {offsetTime.Hours}:{offsetTime.Minutes:d2}:{offsetTime.Seconds:d2} D2V {d2vVsBinPts:f0}\r\n");
                }
                else
                {
                    this.sectionListTextBox.AppendText(
                        $"{++subDataIndex}: Cell {startInfo.CellType} Pos {startInfo.FilePosition} Audio {audio:f0} Video {video:f0} D2V {d2vVsBinPts:f0}\r\n");
                }
            }
        }
        else
    ...
    string CalculateSubFilename()
    {
        ...
        {
            return $"{Path.GetFileNameWithoutExtension(this.data.SelectedSubtitleBinaryFile)} T{this.data.SelectedSubtitleStreamId:x} {this.data.SelectedSubtitleDescription}";
        }
    }
