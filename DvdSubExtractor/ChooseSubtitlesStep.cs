...
public partial class ChooseSubtitlesStep : UserControl, IWizardItem
{
    ...
    public void Initialize(ExtractData data)
    {
        ...
        this.data.NewStepInitialize(this.IsComplete, this.data.Programs.Count != 0,
            this.HelpText, [typeof(LoadFolderStep)]);
    }
    ...
    class SubtitleListItem
    {
        public SubtitleListItem(SubtitleStream stream, SubtitlePacklist pack)
        {
            ...
            this.Description = $"{DvdLanguageCodes.GetLanguageText(this.Stream.Language)} {this.Stream.Format}";
        ...
        public override string ToString()
        {
            return $"Id {this.Stream.StreamId:x2} {DvdLanguageCodes.GetLanguageText(this.Stream.Language)} {this.Stream.Format} ({this.Count} Subtitles {this.ForcedCount} Forced)";
        }
    }
        ...
        if (this.subtitle != null)
        {
            ...
            else
            {
                if (this.pictureBitmap == null)
                {
                    ...
                    this.pictureBitmap.SetResolution(96, 96);
                }
                else
                {
            ...
            this.indexLabel.Text = $"Time {span.Hours}:{span.Minutes:d2}:{span.Seconds:d2}.{span.Milliseconds / 10:d2} Len {spanDuration.Seconds}.{spanDuration.Milliseconds / 10:d2}";
            this.recursiveIndexUpDown = true;
            try
            {
    ...
    private void ChooseSubtitlesStep_Load(object sender, EventArgs e)
    {
