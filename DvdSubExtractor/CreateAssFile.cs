...
public class CreateAssFile
{
    ...
    private CreateAssFile(ExtractData data, SortedDictionary<double, double> ptsOffsets, CreateSubOptions options)
    {
        ...
        if(this.data.WorkingData.VideoAttributes.HorizontalResolution != 0)
        {
            ...
            {
                headers[4] = $"PlayResX: {this.data.WorkingData.VideoAttributes.HorizontalResolution - Math.Max(0, (2 * this.options.Crop.X))}";
            }
            headers[5] = $"PlayResY: {this.data.WorkingData.VideoAttributes.VerticalResolution - Math.Max(0, (2 * this.options.Crop.Y))}";
        }
        else
        {
            headers[4] = $"PlayResX: {SubConstants.DefaultDvdHorizontalPixels}";
            headers[5] = $"PlayResY: {SubConstants.DefaultDvdVerticalPixels}";
        }
        headers[7] = "Audio File: ";
        headers[8] = "Video File: ";
    }

    static readonly string[] AssHeader =
    [
        "[Script Info]",
        "Title: Default file",
        "ScriptType: v4.00+",
    ...
    {
        return $"Style: {name},{fontName},{fontSize},{CreateAssStyleColor(textColor)},&H000000FF,{CreateAssStyleColor(borderColor)},{CreateAssStyleColor(shadowColor)},{(isBold ? "1" : "0")},0,0,0,{scaleX},100,0,0,1,{borderWidth.ToString("f1", CultureInfo.InvariantCulture.NumberFormat)},{shadowWidth.ToString("f1", CultureInfo.InvariantCulture.NumberFormat)},2,{horizontalMargin},{horizontalMargin},{verticalMargin},1";
    }

    static readonly string[] AssHeaderAfterStyle =
    [
        "",
        "[Events]",
    ...
    {
        string timeLine = $"Dialogue: {prefix}0,{timeStart.Hours:d1}:{timeStart.Minutes:d2}:{timeStart.Seconds:d2}.{timeStart.Milliseconds / 10:d2},{timeEnd.Hours:d1}:{timeEnd.Minutes:d2}:{timeEnd.Seconds:d2}.{timeEnd.Milliseconds / 10:d2},{fontStyleName},Unknown,0000,0000,0000,,";
        return timeLine;
    }
    ...
    private void CreateSubtitleFile()
    {
        ...
        foreach(KeyValuePair<int, IList<SubtitleLine>> entry in work.AllLinesBySubtitle)
        {
            if(entry.Value.Count != 0)
            {
                ...
                Debug.WriteLine($"Pts {subData.Pts:f0} Adjust {ptsAdjustment:f0}");
        ...
        int scaleX = (this.data.WorkingData.VideoAttributes.HorizontalResolution == SubConstants.DefaultDvdHorizontalPixels)
            ? this.data.WorkingData.VideoAttributes.AspectRatio switch
            {
                VideoAspectRatio._16by9 => SubConstants.FontScaleX16x9Dvd,
                VideoAspectRatio._4by3 => SubConstants.FontScaleX4x3Dvd,
                _ => SubConstants.FontScaleNormal,
            }
        ...
        Size videoSize, IList<Color> rgbPalette, Color mostCommonColor, bool positionAllLines, RemoveSDH removeSDH)
    {
        ...
        for(int index = 0; index < subLines.Count; index++)
        {
            ...
            else
            {
                ...
                {
                    positionTag = $@"{{\an4\pos({state.LeftEdge + originOffset.X},{state.MidY + originOffset.Y})}}";
                }
