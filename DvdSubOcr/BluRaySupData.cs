...
public static class SupDecoder
{
    ...
    static void FillData(Span<byte> pixData, int index, int width, int stride, byte value, int count)
    {
        int y = index / width;
        int x = index % width;
        while (count > 0)
    ...
    public static unsafe void DecodeImage(PcsObject pcs, IList<OdsData> data, int[] paletteBucket, IntPtr bitmapPtr, int stride)
    {
        ...
        Span<byte> pixData = new Span<byte>((void*)bitmapPtr, h * stride);
        ...
        do
        {
            ...
            if (b == 0)
            {
                ...
                else
                {
                    ...
                    else if ((b & 0xC0) == 0x80)
                    {
                        ...
                        size = (b - 0x80);
                        b = buf[index++] & 0xff;
                        FillData(pixData, ofs, w, stride, (byte)b, size);
                        ofs += size;
                        xpos += size;
            ...
            {
                FillData(pixData, ofs, w, stride, (byte)b, 1);
                ofs++;
                xpos++;
                paletteBucket[b]++;
            }
