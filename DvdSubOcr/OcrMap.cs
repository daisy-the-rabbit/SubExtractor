...
public class OcrMap
{
    ...
    public void AddSplit(string fullEncode, OcrSplit split1, OcrSplit split2, int movieId)
    {
        ...
        this.ocrSplits[fullEncode] = new SplitMapEntry(split1, split2, [movieId]);
    }
    ...
    {
        string storageDir = useProgramExeDirectory ? Path.GetDirectoryName(Environment.ProcessPath) :
    ...
    public void Load()
    {
        ...
        using (FileStream fileStream = new FileStream(StorageFile, FileMode.Open, FileAccess.Read))
        {
            using (BinaryReader reader = new BinaryReader(fileStream, Encoding.Unicode))
            {
                ...
                string stats = $"Dups {duplicateMovieIds.Count} Ocrs {dupOcrsFound} OcrsUsed {dupOcrsUsed} Splits {dupSplitsFound} SplitsUsed {dupSplitsUsed}";
