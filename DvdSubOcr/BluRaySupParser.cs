...
public static class BluRaySupParser
{
                ...
                case 0x17: // Window display
                    if (latestPcs != null)
                    {
                        ...
                        for (int nextWindow = 0; nextWindow < windowCount; nextWindow++)
                        {
                            ...
                            log.AppendLine($"WinId: {windowId}, X: {x}, Y: {y}, Width: {width}, Height: {height}");
                            offset += 9;
                        }
                    }
                    break;
    ...
    static PcsData ParsePicture(byte[] buffer, SupSegment segment)
    {
        ...
        sb.Append($"CompNum: {pcs.CompNum}, Pts: {PtsToTimeString(pcs.Pts)}, State: {pcs.CompositionState}, PalUpdate: {pcs.PaletteUpdate}, PalId {pcs.PaletteId}");
        ...
        else
        {
            ...
            for (int compObjIndex = 0; compObjIndex < compositionObjectCount; compObjIndex++)
            {
                ...
                sb.AppendLine();
                sb.Append($"ObjId: {pcsObj.ObjectId}, WinId: {pcsObj.WindowId}, Forced: {pcsObj.IsForced}, X: {pcsObj.Origin.X}, Y: {pcsObj.Origin.Y}");
                offset += 8;
            }
        }
